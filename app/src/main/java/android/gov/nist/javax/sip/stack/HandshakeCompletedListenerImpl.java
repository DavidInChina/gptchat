...
public class HandshakeCompletedListenerImpl implements HandshakeCompletedListener {
    ...
    public class HandshakeWatchdog extends SIPStackTimerTask {
        ...
        public void runTask() {
            StackLogger stackLogger = HandshakeCompletedListenerImpl.logger;
            stackLogger.logInfo("closing socket " + this.socket + " on sslhandshaketimeout");
            try {
                this.socket.close();
            } catch (IOException unused) {
                HandshakeCompletedListenerImpl.logger.logInfo("couldn't close socket on sslhandshaketimeout");
            }
            StackLogger stackLogger2 = HandshakeCompletedListenerImpl.logger;
            stackLogger2.logInfo("socket closed " + this.socket + " on sslhandshaketimeout");
        }
    ...
    public void startHandshakeWatchdog() {
        if (this.watchdog != null) {
            ...
            stackLogger.logInfo("starting watchdog for socket " + this.watchdog.socket + " on sslhandshake " + this.sipStack.getSslHandshakeTimeout());
