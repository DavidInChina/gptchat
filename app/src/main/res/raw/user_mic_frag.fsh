...
ColoredSDF applyBottomAlignedBarsAndMicState(ColoredSDF sdf, SDFArgs args, vec4 avgMag) {
    float d = 1000.0;
    int barCount = 5;
...
void main() {
    ...
    micArgs.amount = min(stateListen + stateListenIntently + stateListenGrace, 1.0);
    ...
    micSdf = applyBottomAlignedBarsAndMicState(micSdf, micArgs, userMagnitudes);
